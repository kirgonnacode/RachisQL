...
- Пиши максимально ПРОСТОЙ SQL. Не оборачивай в подзапрос то, что можно
  сделать одним плоским запросом с агрегатной функцией (SUM/COUNT/AVG).
  Вложенные подзапросы используй только когда без них реально нельзя обойтись.
...
async def generate_sql(
    ...
    usage_pct = (prompt_tokens / OLLAMA_NUM_CTX * 100) if OLLAMA_NUM_CTX else 0
    logger.info(
        "Промпт: %d токенов из %d доступных (%.0f%% контекстного окна)",
        prompt_tokens, OLLAMA_NUM_CTX, usage_pct,
    )
    if usage_pct > 80:
        logger.warning(
            "Промпт занимает больше 80%% контекстного окна (%d/%d токенов) - "
            "риск автоматической обрезки контекста, стоит поднять OLLAMA_NUM_CTX или сократить SAMPLE_ROWS_COUNT",
            prompt_tokens, OLLAMA_NUM_CTX,
        )       
